...
<div {{ $attributes->class(['relative flex flex-col gap-5 ' . $rounded ?? 'rounded-lg']) }}>
    @if ($inline)
        <div class="flex gap-x-5 px-2">
            @if ($docu->tags->isNotEmpty())
                <div class="flex flex-col items-center justify-around gap-y-1">
                    {{-- <div class="w-6">
            ...
            <div class="flex flex-col justify-center gap-y-1">
                <span class="w-fit text-ellipsis overflow-hidden whitespace-nowrap">
                    {{ $docu->title }}
                </span>
            ...
            <div class="ml-auto flex justify-end gap-0.5 flex-wrap">
                @foreach ($docu->fields as $field)
                    <div>
                        <flux:badge size="sm" color="{{ $field->color }}">{{ $field->field }}</flux:badge>
                    </div>
                @endforeach
            </div>
        </div>
        ...
        <div class="flex flex-col items-center justify-center px-5 py-3">
            ...
            @if (isset($docu->subtitles))
                <div class="flex items-center justify-between w-full h-fit">
                    <span class="flex items-center gap-1 text-sm">
                        <flux:icon.chat-bubble-left variant="outline"
                            class="size-4 bg-white! text-zinc-900" />Sous-titres
                    </span>
            ...
            <div class="flex flex-col cursor-pointer" x-data="{ expanded: false }">
                ...
                <div class="bg-zinc w-[94%] self-end border-zinc-200 mt-2"
                    ...
                    @foreach ($docu->from as $production_house)
                        <a wire:navigate href='/production_house/{{ $production_house->id }}'
                            class="text-sm text-zinc-500 list-item">{{ $production_house->name }}</a>
                    @endforeach
                </div>
            </div>
            <div class="flex flex-col cursor-pointer" x-data="{ expanded: false }">
