...
use App\Domains\Docus\Actions\EditDocu;
...
new class extends Component {
    ...
    public function mount(Docu $docu)
    {
        ...
        $this->links = $docu->see_at->toArray();
        $this->comment = $docu->comment;
    }
    ...
    public function save(EditDocu $edit)
    {
        $this->validate($this->rules());
        $edit->execute(Auth::user(), $this->docu, [
            'title' => $this->title,
        ...
        $this->redirect('/docu/' . $this->docu->id, navigate: true);
    }
};
?>
    ...
    <flux:modal variant="flyout" name="create-docu" class="max-w-max">
        <form class="space-y-6" wire:submit.prevent='save'>
            ...
            <flux:fieldset>
                ...
                <div class="grid md:grid-cols-2 gap-x-5 space-y-3 items-baseline">
                    ...
                    <flux:field>
                        ...
                        <livewire:pill-box name="target" :datas="DocuTarget::toArray()" :one_result="true" :selected="[$target]" />
                    </flux:field>
                </div>
                {{-- COMMENT --}}
                <flux:field>
            ...
            <div class="flex justify-between">
                ...
                <flux:button variant="primary" wire:click='save()' color="green" class="cursor-pointer">Editer
                </flux:button>
            </div>
        </form>
    </flux:modal>
